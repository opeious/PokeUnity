...
public class PokemonAssetImporter : AssetPostprocessor
{
    ...
    private static void UpdatePokemonLoader ()
    {
        var x = AssetDatabase.FindAssets ("t:Model");
        var y = new List<GameObject> ();
        foreach (var z in x) y.Add (AssetDatabase.LoadAssetAtPath<GameObject> (AssetDatabase.GUIDToAssetPath (z)));
    ...
    private void HandleAnimationClips (ModelImporter modelImporter)
    {
        ...
        foreach (var animClip in animClips) animClip.loopTime = true;
    ...
    private void MoveToAppropriateFolder (ModelImporter modelImporter)
    {
        ...
        var filesInNewPath = Directory.GetFiles (newFolderPath);
    ...
    private void HandleTextures (GameObject g, ModelImporter modelImporter)
    {
        ...
        foreach (var mat in renderer.sharedMaterials) {
            ...
            if (!textureAssetPath.Contains (".tga")) textureAssetPath = textureAssetPath.Replace (".png", ".tga.png");
            ...
            if (File.Exists (textureAssetPath)) FixTexture (textureAssetPath);
            var texture2D = (Texture2D) AssetDatabase.LoadAssetAtPath (textureAssetPath, typeof(Texture2D));
            ...
            if (File.Exists (detailAssetPath)) FixTexture (detailAssetPath);
            texture2D = (Texture2D) AssetDatabase.LoadAssetAtPath (detailAssetPath, typeof(Texture2D));
            if (texture2D != null) newMat.SetTexture ("_DetailMap", texture2D);

            var normalAssetPath = textureAssetPath.Replace ("1.tga", "Nor.tga");
            if (File.Exists (normalAssetPath)) FixTexture (normalAssetPath);
            texture2D = (Texture2D) AssetDatabase.LoadAssetAtPath (normalAssetPath, typeof(Texture2D));
            if (texture2D != null) newMat.SetTexture ("_NormalMap", texture2D);
            ...
            if (iris) {
                ...
                if (texture2D != null) newMat.SetTexture ("_MaskMap", texture2D);
                newMat.SetFloat ("_AlphaClipThreshold", 0.1f);
            }


            if (created) AssetDatabase.CreateAsset (newMat, matPath);
