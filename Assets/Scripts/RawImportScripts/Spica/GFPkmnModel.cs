        ...
        public static H3D OpenAsH3D(Stream Input, GFPackageExtensions.Header Header, H3DDict<H3DBone> Skeleton = null)
        {
            ...
            switch (MagicNum)
            {
                case GFModelConstant:
                    ...
                    GFPackageExtensions.Header PSHeader = GFPackageExtensions.GetPackageHeader(Input);

                    foreach (GFPackageExtensions.Entry Entry in PSHeader.Entries)
                    {
                    ...
                    {
                        GFPackageExtensions.Header PCHeader = GFPackageExtensions.GetPackageHeader(Input);

                        foreach (GFPackageExtensions.Entry Entry in PCHeader.Entries)
                        {
                            Input.Seek(Entry.Address, SeekOrigin.Begin);
                ...
                case GFTextureConstant:
                    ...
                    foreach (GFPackageExtensions.Entry Entry in Header.Entries)
                    {
                        Input.Seek(Entry.Address, SeekOrigin.Begin);
